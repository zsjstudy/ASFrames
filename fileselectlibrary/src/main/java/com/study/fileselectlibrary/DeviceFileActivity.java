...
public class DeviceFileActivity extends AppCompatActivity {
    ...
    //默认最大选择文件数量
    private final static int FILE_SELECT_MAX_COUNT = 1;

    private int defaultCount = FILE_SELECT_MAX_COUNT;
    private final static int CHOOSE_FILE_BACK_CODE = 300;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == FILE_CODE) {
            if (resultCode == RESULT_CODE) {
                ...
                if (resultFileList != null && resultFileList.size() != 0) {//如果没有选中文件就不退出
                    backLastLevel();
                }
            }
        }
